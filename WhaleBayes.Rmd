---
title: "Whale Bayes: On seperating occurrence and behavior in movement ecology"
...
The key value of interest is z, the conditional probability of foraging given occurrence. Here the mean estimate is `r as.numeric(pc %>% filter(par %in% "z") %>% summarize(mean(value)))`
...
system.time(jagM<-jags.parallel(model.file = "Bayesian/Env.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=10000,n.burnin=9900,n.thin=1,DIC=FALSE))
...
### Compare to tradiditonal moveHMM

Compare the same data and simulated function when entered into a traditional movement model. We use the moveHMM package for easy comparison. Our 'track' is an animal passing into every cell, randomly. We want to stress that in no way are we degraded the very excellent work by many movement ecologists, and especially the designers of the moveHMM package. The model being tested was not developed with such a mixture in mind. Nor are we claiming that the divide among occurrence and behavior was unknown. Instead we aim to link two parts of ecological thought, which we hope will motivate continued development within these communities. 
