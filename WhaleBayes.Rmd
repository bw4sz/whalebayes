...
The key value of interest is z, the conditional probability of foraging given occurrence. Here the mean estimate is `r round(as.numeric(pc %>% filter(par %in% "z") %>% summarize(mean(value))),2)`, very close to the true known value of 0.25 (which we can estimate using bayes rule). As we increase the grid size (more data), we would converge on the true answer.
