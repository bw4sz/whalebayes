...
To help illustrate this example, the coins have different margin total, but are randomly distributed in the system

```{r}
r<-raster(nrows=30,ncol=30)
...
The key value of interest is z, the conditional probability of foraging given occurrence. Here the mean estimate is `r round(as.numeric(pc %>% filter(par %in% "m") %>% summarize(mean(value))),2)`, very close to the true known value of 0.25 (which we can estimate using bayes rule).

$$ P(B=2|Occ=1) = \frac{P(B=2,Occ=1)}{Occ=1} $$
...
system.time(jagM<-jags.parallel(model.file = "Bayesian/Env.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=100000,n.burnin=35000,n.thin=1,DIC=FALSE))
...
#get x y locations
dat<-data.frame(cell=1:ncell(env),occ,beh)
```
