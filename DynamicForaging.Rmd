...
#single index
splitpc[c("alpha_mu","theta")]<-lapply(splitpc[c("theta")],function(x){
...
pc[pc$parameter %in% "gamma",]$MonthA<-month.name[as.numeric(pc$Month[pc$parameter %in% "gamma"])]
pc$MonthA<-factor(pc$MonthA,levels=month.name)

ggplot(data=pc[pc$parameter %in% c("gamma"),],aes(x=value,fill=MonthA)) + geom_density(alpha=0.5) + theme_bw()  + ggtitle("Estimated parameters") + facet_wrap(~Behavior)
...
#format gamma df
gt<-gammadf %>% spread(key=Behavior,value=mean) %>% droplevels()
gt<-split(gt[,2:3],gt$MonthA)
...
mt<-lapply(gt,function(k){
  ...
  for(x in 1:ind){
      mxy[[x]]<-traj(gamma=as.numeric(k),theta=c(0,0),a1_mu=c(0.1,0.9),a1_sd=c(0.01,0.01),total_time = step.ind[x],step_length=step_length)
  } 
  return(mxy)
})
...
ggplot(mt,aes(x=x,y=y)) + theme_bw()  + geom_path(aes(col=State,group=Animal),size=1)+ facet_wrap(~Month,scales='free') + theme_classic() + theme(axis.text.x=element_blank(),axis.text.y=element_blank()) + labs(x="",y="")
```
...
ggplot(mr,aes(x=bstate,y=mean,ymin=lower,ymax=upper,col=MonthA)) + geom_pointrange(size=2) + theme_bw() + ggtitle("Average Duration of Behavior") + labs(y="Hours")
